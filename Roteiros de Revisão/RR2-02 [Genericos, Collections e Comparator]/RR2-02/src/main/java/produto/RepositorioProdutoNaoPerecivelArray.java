...
public class RepositorioProdutoNaoPerecivelArray {
	...
	public void atualizar(ProdutoNaoPerecivel produto) {
		if (!existe(produto.getCodigo())){
			throw new RuntimeException("Produto nao se encontra no repositorio");
		} else {
	...
	public void remover(int codigo) {
		...
		for (int i = indice; i < produtos.length - 1; i = i + 1){
	...
	public ProdutoNaoPerecivel procurar(int codigo) {
		for (int i = 0; i < produtos.length; i = i + 1){
			ProdutoNaoPerecivel produtoNaoPerecivel = (ProdutoNaoPerecivel) produtos[i];
